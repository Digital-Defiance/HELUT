...
| **C11** | `Scripts/kiss_hunt.py` + `--exhaust-selftest`, both re-run into fresh logs | **Reproduced the August sparse-model ledger exactly**: 3 recovered daily keys, 72 overlays → 8 survivors vs 5.8 by chance. Its known-shell rehearsal ranked the true key **223 118 / 456 976** (= 26⁴, top 48.8%), recovered 4/72 letters and 0/10 plugs, and printed `NO BREAK`. The current dense/repaired rerun keeps that rank, reaches 25/72 and 5/10, and still prints `NO BREAK` (`logs/enigma-dependent-rerun-20260907T001858Z/manifest.json`) |
