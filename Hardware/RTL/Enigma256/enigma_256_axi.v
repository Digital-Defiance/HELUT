...
//   2. Burst:  arm via CTRL[1], stream 2304 bytes on s_axis_* (enigma_256_axis_tables)
...
module enigma_256_axi (
    ...
    reg [7:0]  reg_data_in;
    reg [7:0]  reg_center_mask;
    reg [63:0] reg_byte_counter;
    reg [7:0]  core_center_mask;
    reg [63:0] core_absolute_byte_counter;
    ...
    reg        core_load_state;
    reg        core_valid_in;
    reg        core_schedule_error_d;
    reg        axis_arm;
    // Delayed stream tuple: all fields remain stable together for the cycle in
    // which valid_in is observed by the core, including optional jitter delay.
    reg        pend_stream;
    reg [7:0]  pend_stream_data;
    reg [7:0]  pend_stream_center_mask;
    reg [63:0] pend_stream_byte_counter;
    ...
    enigma_256_core u_core (
        ...
        .init_r3_pos(reg_r3),
        .init_r4_pos(reg_r4),
        .init_byte_counter(reg_byte_counter),
        .data_in(reg_data_in),
        .center_mask(core_center_mask),
        .absolute_byte_counter(core_absolute_byte_counter),
    ...
    function automatic [31:0] read_mux;
        ...
        begin
            case (addr)
                ...
                8'h30: read_mux = {28'd0, core_schedule_error, axis_done, busy, latched_valid};
                8'h34: read_mux = {31'd0, sca_jitter_en};
                8'h38: read_mux = {20'd0, axis_byte_count};
                8'h3C: read_mux = {24'd0, reg_center_mask};
                8'h40: read_mux = reg_byte_counter[31:0];
                8'h44: read_mux = reg_byte_counter[63:32];
    ...
    always @(posedge aclk or negedge aresetn) begin
        if (!aresetn) begin
            ...
            reg_data_in <= 8'd0;
            reg_center_mask <= 8'd0;
            reg_byte_counter <= 64'd0;
            core_center_mask <= 8'd0;
            core_absolute_byte_counter <= 64'd0;
            ...
            core_valid_in <= 1'b0;
            core_schedule_error_d <= 1'b0;
            pend_stream <= 1'b0;
            pend_stream_data <= 8'd0;
            pend_stream_center_mask <= 8'd0;
            pend_stream_byte_counter <= 64'd0;
            axis_arm <= 1'b0;
            sca_jitter_en <= 1'b0;
            jitter_hold <= 2'd0;
            jitter_lfsr <= 8'hA5;
            ...
            core_load_state <= 1'b0;
            core_valid_in <= 1'b0;
            core_schedule_error_d <= core_schedule_error;
            axis_arm <= 1'b0;
            ...
            // A rejected tuple has no valid_out completion, so retire payload
            // busy exactly once when the core's sticky error first rises.
            if (core_schedule_error && !core_schedule_error_d)
                busy <= 1'b0;
            ...
            if (jitter_hold != 0) begin
                ...
                core_center_mask <= pend_stream_center_mask;
                core_absolute_byte_counter <= pend_stream_byte_counter;
                core_valid_in <= 1'b1;
                pend_stream <= 1'b0;
            end
            ...
            if (wr_hs) begin
                ...
                case (s_axi_awaddr)
                    ...
                    8'h28: begin
                        // Snapshot the complete fixture-v4 stream tuple atomically.
                        pend_stream_data <= s_axi_wdata[7:0];
                        pend_stream_center_mask <= reg_center_mask;
                        pend_stream_byte_counter <= reg_byte_counter;
                        pend_stream <= 1'b1;
                        latched_valid <= 1'b0;
                        busy <= 1'b1;
                        if (sca_jitter_en)
                    ...
                    8'h3C: reg_center_mask <= s_axi_wdata[7:0];
                    8'h40: reg_byte_counter[31:0] <= s_axi_wdata;
                    8'h44: reg_byte_counter[63:32] <= s_axi_wdata;
                    default: ;
                endcase
