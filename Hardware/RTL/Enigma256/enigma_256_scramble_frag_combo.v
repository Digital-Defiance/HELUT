...
// scramble fragment (identity plug → R1…R4 → XOR center → reverse → plug).
// The profile-v4 HMAC-derived center mask is a transported input; this cone
// does not implement or approximate the host-side HMAC schedule. Stand-in
// bijections are not live BRAM tables. Matches Swift `enigma256ScrambleFrag`.
...
module enigma_256_scramble_frag_combo (
    ...
    // Identity plug → four forward stages → XOR center → four reverse → plug.
    ...
    wire [7:0] center_out = r4_out ^ center_mask;

    wire [7:0] r4r_in  = center_out + offset_r4;
