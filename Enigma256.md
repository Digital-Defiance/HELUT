...
- **Red/Blue campaign:** `Scripts/enigma256_rb_campaign.sh` fail-closes on SoftBus `ent` + structured KPA by default (`--no-gates` to skip). `--hard-red` / `--wide` re-score NLFF or NLFF+`lfsr_next_hi` with `--enigma256-tensorlut-expect-hold`. Battery: `Scripts/enigma256_red_battery.sh`. Structured KPA (`--enigma256-structured-kpa`) includes day-only joint hill-climb. Ent: PRNG plaintext (zeros are biased by the un-reflector on purpose).
