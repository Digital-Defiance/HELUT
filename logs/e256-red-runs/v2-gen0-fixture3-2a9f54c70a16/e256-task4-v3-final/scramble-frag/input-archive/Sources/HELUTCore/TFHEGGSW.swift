...
/// Domain/order used to turn encrypted input bits into a blind-rotation address.
package enum LWEInputPackingMode: String, Sendable, Equatable {
    /// Inputs already live in rotation-native `Z_{2N}`; reduce each component while packing.
    case rotationNative = "rotation-native"
    /// Inputs live in the full UInt32 torus; aggregate there, then switch the aggregate once.
    case fullTorusPublicMS = "full-torus-public-ms"
}

/// Pack rotation-native boolean LWE bits into one address: `Σ 2^i · ct_i (mod 2N)`.
/// `twoN` is the **polynomial** torus `2N` (`N = deg`), not `2·n` when `n < N`.
///
/// This standalone API intentionally keeps its historical native-domain default. Full-torus
/// public-MS callers must use `packFullTorusLWEBits` / `packLWEBitsAggregateFirst` instead.
...
/// Pack full-torus LWE bits with wrapping UInt32 arithmetic and no component-wise `mod 2N`.
package func packFullTorusLWEBits(_ bits: [LWECiphertext]) -> LWECiphertext {
    precondition(!bits.isEmpty)
    let dim = bits[0].lweDimension
    precondition(bits.allSatisfy { $0.lweDimension == dim })
    var aggregate = bits[0]
    for i in 1..<bits.count {
        aggregate = addLWE(
            aggregate,
            scaleLWE(bits[i], (1 as UInt32) &<< UInt32(i))
        )
    }
    return aggregate
}

/// Aggregate full-torus inputs first, then nearest-switch that one address into `Z_{2N}`.
package func packLWEBitsAggregateFirst(
    _ bits: [LWECiphertext],
    twoN: Int,
    scale: UInt32
) -> LWECiphertext {
    publicRefreshBit(
        packFullTorusLWEBits(bits),
        twoN: twoN,
        scale: scale
    )
}

/// Explicit-domain packing used by blind-rotation evaluators and diagnostics.
package func packLWEBits(
    _ bits: [LWECiphertext],
    twoN: Int,
    scale: UInt32,
    mode: LWEInputPackingMode
) -> LWECiphertext {
    switch mode {
    case .rotationNative:
        return packLWEBits(bits, twoN: twoN)
    case .fullTorusPublicMS:
        return packLWEBitsAggregateFirst(bits, twoN: twoN, scale: scale)
    }
}
