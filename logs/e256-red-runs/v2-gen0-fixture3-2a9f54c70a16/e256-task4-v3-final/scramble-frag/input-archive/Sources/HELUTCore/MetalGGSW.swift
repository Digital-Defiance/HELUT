...
package enum MetalGGSW {
    ...
    //
    // Full-torus public-MS wires: each LUT packs in UInt32, switches the aggregate
    // once, then uses dynamic X^p so dependent `$lut`s chain without output refresh.
    ...
    /// Evaluate a combinational LUT netlist whose public-MS wires stay in the full torus.
    /// The default aggregates each LUT address in UInt32, switches that aggregate once,
    /// and retains raw extracted outputs for dependent LUTs.
    /// Default lowering is host-scheduled per-LUT BR (tiled-kernel at *N*>64).
    ...
    package static func evaluateTopoNetlistSingleGraph(
        ...
        commandQueue: MTLCommandQueue,
        keySwitchKey: KeySwitchKey? = nil,
        inputPacking: LWEInputPackingMode = .fullTorusPublicMS
    ...
    /// Host-scheduled LUT BRs with explicit native or full-torus input packing.
    private static func evaluateTopoNetlistTiledKernel(
        ...
        commandQueue: MTLCommandQueue,
        keySwitchKey: KeySwitchKey? = nil,
        inputPacking: LWEInputPackingMode
    ) throws -> [Int: LWECiphertext] {
        let params = bootstrapKey.params
        precondition(params.tfhe.glweDimension == 1)
        let n = params.tfhe.polynomialDegree
        ...
        for (jobIndex, job) in jobs.enumerated() {
            ...
            let stored: LWECiphertext
            switch inputPacking {
            case .rotationNative:
                stored = publicRefreshBit(extracted, twoN: twoN, scale: scale)
            case .fullTorusPublicMS:
                stored = extracted
            }
            wires[job.outputWireId] = stored
            produced[job.outputWireId] = stored
        }
        return produced
    }
    ...
    private static func evaluateTopoNetlistFusedGraph(
        ...
        for (jobIndex, job) in jobs.enumerated() {
            ...
            let weightedAggregate = packLWEOnGraph(
                graph: graph,
                bits: inputs,
                lweDimension: lweN,
                batch: batch,
            ...
            let packedAddress: LWETensors
            switch inputPacking {
            case .rotationNative:
                packedAddress = weightedAggregate
            case .fullTorusPublicMS:
                packedAddress = publicRefreshBitOnGraph(
                    graph: graph,
                    lwe: weightedAggregate,
                    scale: scale,
                    twoN: twoN,
                    lweDimension: lweN,
                    batch: batch,
                    name: "packMS\(jobIndex)"
                )
            }
            ...
            let stored: LWETensors
            switch inputPacking {
            case .rotationNative:
                stored = publicRefreshBitOnGraph(
                    graph: graph,
                    lwe: extracted,
                    scale: scale,
                    twoN: twoN,
                    lweDimension: lweN,
                    batch: batch,
                    name: "outputMS\(jobIndex)"
                )
            case .fullTorusPublicMS:
                stored = extracted
            }
            wireTensors[job.outputWireId] = stored
    ...
    private static func publicRefreshBitOnGraph(
        ...
        let halfA = constantPacked(
            graph: graph,
            values: [UInt32](repeating: half, count: lweDimension),
            batch: batch,
            width: lweDimension,
            name: "\(name)_ha"
        )
        let halfB = constantPacked(
            graph: graph, values: [half], batch: batch, width: 1, name: "\(name)_hb"
        )
        let quotientA = graph.division(lwe.a, scaleA, name: "\(name)_aq")
        let quotientB = graph.division(lwe.b, scaleB, name: "\(name)_bq")
        let productA = graph.multiplication(quotientA, scaleA, name: "\(name)_ap")
        let productB = graph.multiplication(quotientB, scaleB, name: "\(name)_bp")
        let remainderA = graph.subtraction(lwe.a, productA, name: "\(name)_arem")
        let remainderB = graph.subtraction(lwe.b, productB, name: "\(name)_brem")
        // Round from the bounded remainder rather than adding half a step to the
        // full torus coefficient. The latter can wrap at UInt32.max before the
        // division and select the wrong native address.
        let carryA = graph.division(
            graph.addition(remainderA, halfA, name: "\(name)_arc"),
            scaleA,
            name: "\(name)_ac"
        )
        let carryB = graph.division(
            graph.addition(remainderB, halfB, name: "\(name)_brc"),
            scaleB,
            name: "\(name)_bc"
        )
        let switchedA = graph.addition(quotientA, carryA, name: "\(name)_as")
        let switchedB = graph.addition(quotientB, carryB, name: "\(name)_bs")
        return LWETensors(
            a: remUInt32(
                graph: graph, value: switchedA, modulus: twoN, width: lweDimension, batch: batch,
                name: "\(name)_ar"
            ),
            b: remUInt32(
                graph: graph, value: switchedB, modulus: twoN, width: 1, batch: batch,
                name: "\(name)_br"
            )
        )
    }
