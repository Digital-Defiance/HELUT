...
enum GermanTrigrams {
    ...
    /// Source-row provenance parallel to `logProbs`; smoothing never changes these bits.
    static var observedEntries: [Bool]? { model?.observedEntries }
    ...
    private static func load() -> Model? {
        ...
        var observedEntries = [Bool](repeating: false, count: 17_576)
        var header = ""
        var loaded = 0
        ...
        for line in text.split(separator: "\n", omittingEmptySubsequences: true) {
            ...
            let tableIndex = letters[0] * 676 + letters[1] * 26 + letters[2]
            counts[tableIndex] = count
