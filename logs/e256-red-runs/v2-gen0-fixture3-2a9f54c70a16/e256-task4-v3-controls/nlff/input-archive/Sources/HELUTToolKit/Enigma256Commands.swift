...
func runEnigma256StructuredKPA() {
    ...
    let defaultCrib =
        "HELUT Enigma256 structured SoftBus KPA long crib — bounded day-only joint hill-climb pressure vector."
    ...
    print("  compatibility: \(profile.compatibilityKey)")
    print("  scope: bounded attack run; not a security claim")
    print("  crib: \(crib.count) B  rounds: \(rounds)")
    print("  hill-climb LFSR (pos+Walzenlage known): \(climbLFSR.bestMatches)/\(crib.count) in \(String(format: "%.1f", climbLFSR.elapsedMs)) ms \(lfsrPressure ? "RECOVERY" : "no recovery")")
    print("  hill-climb positions (LFSR+Walzenlage known): \(climbPos.bestMatches)/\(crib.count) in \(String(format: "%.1f", climbPos.elapsedMs)) ms \(posPressure ? "RECOVERY" : "no recovery")")
    if let joint = climbJoint {
        print("  hill-climb joint (day only): \(joint.bestMatches)/\(crib.count) in \(String(format: "%.1f", joint.elapsedMs)) ms \(jointPressure ? "RECOVERY" : "no recovery")")
    }
