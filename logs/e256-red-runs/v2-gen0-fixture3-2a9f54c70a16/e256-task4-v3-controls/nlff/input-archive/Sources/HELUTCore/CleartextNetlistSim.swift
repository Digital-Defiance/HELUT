...
package final class CleartextNetlistSimulator {
    ...
    package struct LUTCell {
        ...
        /// Original Yosys truth-table spelling (highest address first).
        package let initMSBFirst: String
    ...
    package struct LUTEvaluation: Equatable {
        package let name: String
        package let inputValues: [UInt8]
        package let address: Int
    ...
    package private(set) var lastLUTEvaluations: [String: LUTEvaluation] = [:]
    private let traceLUTs: Bool

    package init(moduleName: String, module: YosysModule, traceLUTs: Bool = false) {
        self.moduleName = moduleName
        self.traceLUTs = traceLUTs
    ...
    package func tick(inputs: [String: [UInt8]]) -> [String: [UInt8]] {
        if traceLUTs { lastLUTEvaluations.removeAll(keepingCapacity: true) }
        ...
        while !pending.isEmpty {
            ...
            for lut in pending {
                if let aValues = resolveBits(lut.aBits, wires: wires) {
                    ...
                    let output = lut.table[mask]
                    wires[lut.yWire] = output
                    if traceLUTs {
                        lastLUTEvaluations[lut.name] = LUTEvaluation(
                            name: lut.name,
                            inputValues: aValues,
                            address: mask,
                            output: output
