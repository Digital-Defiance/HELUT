...
public enum HelutBenchCLI {
    ...
    if CommandLine.arguments.contains("--mulein-future-manifest-emit") {
        runMuleinFutureManifestEmit()
        exit(0)
    }

    if CommandLine.arguments.contains("--mulein-future-control-grade") {
        runMuleinFutureControlGrade()
        exit(0)
    }

    if CommandLine.arguments.contains("--mulein-future-metal-grade") {
        runMuleinFutureMetalGrade()
        exit(0)
    }

    if CommandLine.arguments.contains("--mulein-future-tensorlut-grade") {
        runMuleinFutureTensorLUTGrade()
        exit(0)
    }
    ...
    if CommandLine.arguments.contains("--bench") {
        ...
        fputs("Try: --bench | --bench-encrypted-micro | --mulein-future-manifest-emit | --mulein-future-control-grade | --mulein-future-metal-grade | --mulein-future-tensorlut-grade | --measure-bk-noise | --hardness-table\n", stderr)
        exit(2)
    }

    public static func handles(_ args: [String]) -> Bool {
        let keys = ["--bench", "--bench-encrypted", "--bench-encrypted-micro",
                    "--mulein-future-manifest-emit",
                    "--mulein-future-control-grade", "--mulein-future-metal-grade",
                    "--mulein-future-tensorlut-grade",
                    "--measure-bk-noise", "--hardness-table", "--estimator-export"]
...
public enum HelutBombeCLI {
    ...
    if CommandLine.arguments.contains("--mulein-future-campaign") {
        runP1030680MuleinCampaign()
        exit(0)
    }
