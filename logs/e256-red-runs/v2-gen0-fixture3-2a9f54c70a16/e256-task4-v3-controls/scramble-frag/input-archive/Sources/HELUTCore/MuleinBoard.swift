...
package struct MuleinBoardResult: Sendable {
    package let live: [UInt32]
    package let droppedEdges: [Int]
    package let pairCount: Int
    package let determinedCount: Int

    package var exact: Bool { droppedEdges.isEmpty }

    /// Stable full-width FNV-1a checksum shared with the Future Bank receipt ABI.
    package var liveHash: UInt32 {
        var hash: UInt32 = 2_166_136_261
        for row in live {
            hash = (hash ^ row) &* 16_777_619
        }
        return hash
    }
}
...
package enum MuleinBoard {
    ...
    package static func propagate(
        ...
        guard tolerance >= 0,
              (0..<26).contains(seedLetter), (0..<26).contains(seedValue),
              menu.ends.count == menu.steps.count,
              scramblers.count == menu.ends.count,
              scramblers.allSatisfy({ $0.count == 26 }),
              menu.ends.allSatisfy({
                  (0..<26).contains($0.0) && (0..<26).contains($0.1)
              }),
              (0...13).contains(maxPlugs), (0...13).contains(exactPlugs),
              maxPlugs == 0 || exactPlugs == 0 || exactPlugs <= maxPlugs
        else { return nil }

        func result(live: [UInt32], droppedEdges: [Int]) -> MuleinBoardResult? {
            var determined = 0
            var halfPairs = 0
            for letter in 0..<26 where live[letter] != 0 {
                determined += 1
                if live[letter].trailingZeroBitCount != letter { halfPairs += 1 }
            }
            let pairs = halfPairs / 2
            if maxPlugs > 0 && pairs > maxPlugs { return nil }
            if exactPlugs > 0 {
                if pairs > exactPlugs { return nil }
                let freeLetters = 26 - determined
                let requiredLetters = 2 * (exactPlugs - pairs)
                if freeLetters < requiredLetters { return nil }
            }
            return MuleinBoardResult(
                live: live,
        ...
        // "Tolerance" means up to this many erasures. Always grade the unmodified board first,
        // including its plug budget, before considering any repair provenance.
        if let live = WelchmanBombe.propagate(
            menu: menu, scramblers: scramblers,
            seedLetter: seedLetter, seedValue: seedValue
        ), let exact = result(live: live, droppedEdges: []) {
            return exact
        }
        guard tolerance > 0 else { return nil }

        let indices = Array(menu.ends.indices)
        let maximumDrops = min(tolerance, indices.count)
        guard maximumDrops > 0 else { return nil }

        for dropCount in 1...maximumDrops {
            var chosen = [Int](repeating: 0, count: dropCount)

            /// Enumerate combinations of `dropCount` edge indices, testing each reduced board.
            func recurse(_ depth: Int, _ start: Int) -> MuleinBoardResult? {
                if depth == dropCount {
                    let drop = Set(chosen)
                    var ends: [(Int, Int)] = []
                    var tables: [[UInt8]] = []
                    ...
                    return result(live: live, droppedEdges: chosen.sorted())
                }

                let remaining = dropCount - depth
                guard start <= indices.count - remaining else { return nil }
                for candidate in start...(indices.count - remaining) {
