...
Authored HDL is canonical under \corpus{Hardware/}; checked-in generated
receipts are under \corpus{Generated/}; disposable regeneration belongs under
\corpus{build/hardware/}. Root HDL and netlist names are compatibility entry
points, not generation destinations.
...
\section{Covering-b2 cheaper SING (\cid{57})}
...
Expect $\varepsilon\log_2\approx-110.7$; regex Metal public-ms PASS.
Covering-b4 public-ms and E256 covering-b2 FAIL.

\section{PicoRV \texttt{abc -lut 6} (\cid{58})}
...
Expect $2006$ LUT, PASS ${\sim}1.35\,\mathrm{s}/1$, $32$-bit. $N{=}8$ traps.
Wavefront (\cid{59}): same command ${\sim}0.165\,\mathrm{s}/1$.

\section{PicoRV covering-b2 Q SING FAIL (\cid{60})}
