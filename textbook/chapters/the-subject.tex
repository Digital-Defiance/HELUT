...
Apple Silicon was the \emph{first} engine because the architect needed that
graph hardware to brute-force the $\lambda$-squeeze and synthesis loops ---
not because the subject is a Mac API.
The definition does not name Apple.
A CPU-only restatement of Theorem~1 is
\texttt{python3 Scripts/tensorlut\_math\_ref.py}.
CUDA and other fabrics remain trajectory until they have \claimkind{C} rows
(\corpus{directives/why-apple-silicon.md}).
...
\begin{enumerate}
  ...
  \item \textbf{Honesty.}
        ...
        certificates must ship with every tick, and which sentences does the
        evidence still not support?
\end{enumerate}
...
Edition~\corpusedition{} treats Pillar~I in lecture depth, Pillar~II
Theorem~\ref{thm:tensorlut} (\cid{19}, stated in Chapter~\ref{ch:pillarii}) in
lecture depth with empirics in seminar, Pillar~III in seminar depth, and the grand-challenge / side-channel
...
print without creating a \claimkind{N}.
