...
Do not teach from this PDF.
This chapter is a briefing for a course that does not exist yet: it is not
in the ACM/IEEE curriculum guidelines, and this book is not ready to put
in front of students.
The pages that follow are an aspirational skeleton---grading weights,
tracks, a fifteen-week outline---so that when humans have vetted and
filled the rest of the volume, the pedagogy has a place to live.
Until then, treat this chapter as a design note, not an instructor's
manual.
...
\begin{itemize}
  \item \textbf{Lecture} Parts~II--III (foundations and Pillar~I), the
        honesty chapter, and Theorem~\ref{thm:tensorlut} (\cid{19}), which is
        stated in Chapter~\ref{ch:pillarii} --- this chapter only refers to it.
...
\begin{enumerate}
  \item Diff \corpus{directives/claim-sheet.md} against the epoch printed on
        the title page.
  \item If a new \claimkind{C} row appeared, assign the matching lab receipt as a
        one-page addendum; do not pretend the old PDF contained it.
  \item If an \claimkind{H} closed, tell the class explicitly: ``this asterisk is
...
\begin{itemize}
  ...
  \item Speculative systems (encrypted LLM firewalls, self-modifying dark
        ...
        not implementation homework, until they have \claimkind{C} rows.
\end{itemize}
