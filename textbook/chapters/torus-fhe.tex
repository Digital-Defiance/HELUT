...
HELUT's current BK encrypt is noiseless ($B_{\mathrm{bk}}=0$); production
must fill a measured $\sigma_{\mathrm{BK}}$ (\hid{4}). \\
\bottomrule
\end{tabular}
\end{center}
...
\emph{depth} story.
Students who say ``we have noisy BK'' because the certificate \emph{type}
exists are failing \hid{4}.
\end{warning}
