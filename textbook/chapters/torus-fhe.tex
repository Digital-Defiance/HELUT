...
calibrated $\sim 176$ and is \emph{not} a lattice-estimator attack cost until
Sage fills the pending JSON (\hid{1}).
...
\reproduced{H2 closed}{Multi-LUT encrypted full\_adder SING PASSes at
$N\in\{256,512,1024\}$. The bug was arity-3 packing overflowing $256\cdot 2N$
headroom; the fix is rotation-native encrypt plus \texttt{packLWEBits} reduce
mod $2N$, with \texttt{rotationPower} preferring $Z_{2N}$ representatives.}
...
Default Metal SING still encrypts BK with $e=0$.
\cid{22} fills $B_{\mathrm{bk}}/\hat\sigma$ from identity-LUT residuals
on a covering gadget ($N=8$: inject $B=64\to\hat\sigma\approx 6396$;
$N=128$: $\hat\sigma\approx 1.47\times 10^6$, still $<\delta/2$, Gaussian
$\varepsilon\log_2\approx -23.5$ not $-64$).
$\ell=1$ \texttt{booleanPublicMS} cannot carry BK noise (\hid{4}). \\
\bottomrule
\end{tabular}
\end{center}
...
\emph{depth} story.
\cid{22} measures residuals; it does not put noisy BK on the $N=1024$ Metal
SING path.
Students who say ``we have production noisy BK'' from the $N=8$ table are
failing the remaining \hid{4} asterisk.
\end{warning}
...
Classical target & $\ge 128$ bits & HELUT est.\ $\approx 176$; \hid{1} \\
$B_{\mathrm{bk}}$ & 0 at $N=1024$ SING; measured at covering $N=8$, $N=128$ & \hid{4} \\
...
honestly. Using demo $N$ for a ``secure encrypted CPU'' slide is a \nid{}.

\growswhen{SageMath fills \corpus{logs/helut-estimator-results.json} and
\hid{1} can be replaced by an estimator table in this section.}
