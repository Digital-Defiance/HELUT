...
The structural theorem of this chapter is lecture-safe (\cid{19}).
Shatter / hold grades and campaign empirics remain seminar-depth and are
\emph{not} the theorem~\cite{helut-tensorlut-thm}.

\growswhen{A completeness theorem: that melt recovers cryptographic structure
combinatoric sieves miss, for a stated class of netlists.
Theorem~1 does not claim that.
...
Still not melt completeness.The named lemmas are
...
TensorLUT grades are not a campaign decrypt (\nid{}, \hid{6}).
\end{warning}
...
The loop is a generative compiler pitting continuous optimization against
discrete physical limits~\cite{helut-release}.
