...
\hid{4} Noisy BK
  & \cid{52}--\cid{54} covering-b1; \cid{57} covering-b2 $\varepsilon$ + regex SING @ $N{=}1024$;
    covering KS reaches $n{=}512$ (\cid{69}, the old $n{=}512$ FAIL withdrawn)
...
Campaign catalog
  & Middle ring $\neq A$ tested under VIII-fast (24/24 dead; 294 WO open); catalog parked @417
...
Encrypted determinism
  & Fixed 2026-08-15: input encryption walked a \texttt{Dictionary} while drawing
    from the shared RNG, so ciphertexts varied per process
  & End-to-end determinism assertion in CI (byte-identical ciphertexts across
    processes at a fixed seed), not just the in-process guard
    (\S\ref{sec:determinism}). \\
$\varepsilon$ sample efficiency
  & \cid{22} resolved at $N{=}128$ ($-18.0$, 95\% bound $-16.1$, $n{=}512$);
    $N{=}1024$ costs $155\,\mathrm{s}$/trial so $n{\approx}8450$ is ${\sim}15$ days
  & Sample all $N$ accumulator coefficients instead of the one extracted residual
    (${\sim}1000\times$ more samples per bootstrap). Until then \cid{35}'s bar is
    supportable only at inject $B\le 4$ (\S\ref{sec:eps-samples}). \\
Vertex maximizers
