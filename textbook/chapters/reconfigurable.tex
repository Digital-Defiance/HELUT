...
Under \emph{additive torus / mock} encoding, XOR is a vector add (free).
AND is a LUT (or a PBS).
On the encrypted GGSW path, both gates are blind-rotates. Do not lecture
``XOR is free in FHE'' from this paragraph.
...
path and \cid{45}--\cid{51} on encrypted demo $N{=}8$ at $4.0$ bits; the
encrypted path's production-$N$ multi-LUT workhorse is the adder, \cid{6}
and \cid{20}; covering noisy adder at the same $N$ is \cid{52}.)
\end{exercise}
