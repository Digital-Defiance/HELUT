...
NTT persist (\cid{18}) & \textbf{0.433} & $15.6\,\mathrm{s}$ ($1.95$\,s/row) & 148\,MiB \\
CPU SING full\_adder & --- & ${\sim}52\,\mathrm{s}$ & --- \\
\bottomrule
\end{tabular}
\end{center}

\hedgebox{H3}{NTT persist wins the microbench and loses boolean SING against
schoolbook persist in this epoch ($15.6\,\mathrm{s}$ vs $12.2\,\mathrm{s}$)
because of sequential in-tile NTT/iNTT round-trips.
Crypto $\ell=2$ SING is not re-timed.
Schoolbook is gone from the tiled \emph{hot path} (\texttt{ring=ntt}); SING
is not yet an NTT victory. Quote both sentences together.}
...
Together they are a SoftBus-native torus compiler: staged IR $+$ ring kernels
$+$ cached executables---instead of a schoolbook megagraph hoping the GPU
will eventually appear.

\growswhen{Fewer in-tile NTT/iNTT round-trips so boolean SING beats
\cid{17}'s $12.2\,\mathrm{s}$; crypto $\ell=2$ SING re-timed; then \hid{3}
can shrink.}
