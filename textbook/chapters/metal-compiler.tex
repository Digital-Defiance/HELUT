...
\negativebox{fused $N=1024$ DNF --- no \claimkind{C} row}{Schoolbook-in-\texttt{MPSGraph}
at $N=1024$ spent $11.6\,\mathrm{h}$ on host encode, never reached the GPU, and
died \texttt{SIGTERM 143}. A compiler that does not finish is a measurement, not
an anecdote --- but it is a \emph{negative}, it has no claim-sheet row, and it
must never be read as a reproduced result.}
...
Why this GPU first: the laboratory was an M-series Mac pushed to extremes
so the $\lambda$-squeeze could run at all
(\corpus{directives/why-apple-silicon.md}).
That does not freeze the math to Apple.
