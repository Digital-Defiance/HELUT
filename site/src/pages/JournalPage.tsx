          ...
          <div className="timeline">
            ...
            <article className="tl-item">
              ...
              <div className="prose">
                ...
                <p>
                  <strong>DONE, no break</strong>, 73/73, exited 2026-09-25T04:59:39Z. Menus 66–73: 5,284,950 raw stops, 40 physical, best IC 0.038, tail −4.629. Menu 62's head still misses the whole-message bar (IC 0.049, tail −4.387). Not a plaintext. Middle ring stayed A. Three <code>FUNKSPRUCHUEBUNG</code> quarantine rows are prefix hits (tails −5.256, −4.534, −4.733 on the whole message) and are not an escalate. The catalog VIII-fast continuation is <strong>done, no break</strong>, 451/451, exited 2026-09-25T23:57:03Z (642,562,788 raw stops, best physical IC 0.041, tail −4.488; 42 of 336 orders). A naval-register trigram, held out from the message it scores, sits at 3.14–3.33 bits/letter against a dense median of 4.36. Annealing a fresh board on the 55 UEBUNG near-misses and every logged physical rotor setting (235 settings) reaches a best of 4.069 bits/letter. None reach that held-out band. The catalog sweep still uses the dense trigram. Log <code>logs/campaign-thetis-register-rings.log</code>.
                </p>
              </div>
            </article>
            ...
            <article className="tl-item">
              <div className="when">Phase 73: Tafel A on the corpus</div>
              <h3>ACH belongs to this message alone. The radio stories do not add a key.</h3>
              <div className="prose">
                <p>
                  The full Quelle Tafel A decodes all 48 corpus indicators under the same layout that turns <code>VROL NMKA</code> into <code>ACH</code>. No kenngruppe is shared by two nets. <code>ACH</code> appears only on P1030680, so no solved message in the scrape carries a key we could reuse. <code>DUZ</code> is P1030690, on the Potsdam net, at K-Buch column 12. Column 12 and column 645 are different blocks. A complete table decodes every indicator; it does not by itself prove the sister nets used Tafel A. That proof is a K-Buch column for each decoded kenngruppe.
                </p>
                <p>
                  Selm Merel Wenselaers separated five stories: ordinary Thetis with the key missing; Thetis meant for another boat and only heard by U-534; a repeat of an earlier encipherment; an Offizier text under a Thetis outer key; a stale or wrong key. Hearing someone else's Thetis signal uses the same daily key. An earlier day needs that day's Tafel, and only Tafel A is transcribed. 3 May stays unencoded. No other Thetis key is in hand. The Gruppenwelle parallel is logged with her own caveat: the Thetis link is unproven. The sheet fields that choose among these stories are reception time, frequency, Leitnummer, callsign, repeat mark, and the same fields on the neighbouring sheets. ARGUS v3.1 records that its crib export had been only the Hörenberg scrape. The other family in that bundle is a 1943 U-466 war-diary transcription and a Nixe sheet. Neither is queued. ARGUS v3.2 still offered a crib command and is superseded by v4, which emits no crib: all 121 candidates are marked ineligible. P1030680 remains unbroken.
                </p>
              </div>
            </article>
          </div>
