...
| **C32** | Track A approx candidate: `.crypto` tiny inject @ *N*=1024 | `--measure-bk-noise --degree 1024 --trials 4 --bk-noise 1` (and `2`) | `cryptoPublicMS` *B*≥1 **undecodable**. `.crypto` *B*=1 **decodable** εlog2≈**−32.2**; *B*=2 εlog2≈−1.3. Better than **C26** *B*=4 (ε≈−1); still not −64. · `logs/helut-noisy-bk-measure-n1024-B1.log` |
...
| **H4** | Noisy BK in product path | **C27**/**C29**: exact covering+*g₀*=*δ* never at *N*=1024 for any power-of-two *q*. **C31**: incomplete gap = 10 uncovered bits; approx candidate = covering `.crypto`. **C32**: `.crypto` *B*=1 ε≈2⁻³² (decodable); still not 2⁻⁶⁴. **Track B Metal closed:** **C28**/**C30**. **Track A** Metal SING stays `cryptoPublicMS` + *e*=0 (**C26**). Next: shrink ε on covering-non-δ path, or true approx MS with graded *g₀*≈*δ*. |
