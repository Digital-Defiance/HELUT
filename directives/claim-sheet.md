...
| **H4** | Noisy BK in product path | **C27**/**C29**: exact covering+*g₀*=*δ* never at *N*=1024 for any power-of-two *q*. **C31**–**C33**: Track A approx = covering `.crypto` (*g₀*≠*δ*). Metal SING with inject *B*=1 **PASS** (secret + public-ms covering). Measured Gaussian ε at *B*=1 ≈2⁻⁸·⁴ (8 trials) — **not** 2⁻⁶⁴. `cryptoPublicMS` + noise still fails. Throughput *e*=0 `cryptoPublicMS` SING (**C21**) remains the noiseless bar. Next: ε≤2⁻⁶⁴ on covering-non-δ. |
